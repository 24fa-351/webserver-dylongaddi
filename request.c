...
bool read_request_line(Request* req, int fd)
{
  ...
  if (line == NULL)
  {
    ...
    num_parsed = sscanf(line, "%s %s %s%n", req->method, req->path, req->version, &length_parsed); //%n saves num of characters parsed into length parsed
    if (num_parsed != 3 || length_parsed != strlen(line)) {
